...
router.get('/leaderboard', (_req: Request, res: Response, next: NextFunction) => {
  try {
    const rows = db.select({
      ...
      .leftJoin(
        resultsTable,
        and(eq(resultsTable.player_id, playersTable.id), isNull(resultsTable.deleted_at)),
      )
      .leftJoin(
        sessionsTable,
        and(eq(sessionsTable.id, resultsTable.session_id), isNull(sessionsTable.deleted_at)),
      )
  ...
  try {
    ...
    const rows = db.select({
      ...
      .innerJoin(
        sessionsTable,
        and(
          eq(sessionsTable.id, resultsTable.session_id),
          eq(sessionsTable.game_id, gameId),
...
router.get('/most-played', (_req: Request, res: Response, next: NextFunction) => {
  try {
    const rows = db.select({
      ...
      .leftJoin(
        sessionsTable,
        and(eq(sessionsTable.game_id, gamesTable.id), isNull(sessionsTable.deleted_at)),
      )
      .leftJoin(
        resultsTable,
        and(eq(resultsTable.session_id, sessionsTable.id), isNull(resultsTable.deleted_at)),
      )
...
router.get('/head-to-head', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const player1 = db.select().from(playersTable)
      .where(and(eq(playersTable.id, p1), isNull(playersTable.deleted_at)))
      .get();
    const player2 = db.select().from(playersTable)
    ...
    const sharedSessions = db.select({
