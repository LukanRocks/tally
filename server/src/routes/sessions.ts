...
router.get('/:id', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const session = db.select().from(sessionsTable)
...
router.post('/', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const createSession = sqlite.transaction(() => {
      const [session] = db.insert(sessionsTable).values({
        game_id,
        played_at,
        notes: notes || null,
      }).returning().all();
...
router.delete('/:id', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const existing = db.select().from(sessionsTable)
      .where(and(eq(sessionsTable.id, id), isNull(sessionsTable.deleted_at)))
      .get();
