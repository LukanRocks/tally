...
router.get('/:id', (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const win_rate = stats && stats.total_sessions > 0
      ? Math.round((stats.wins / stats.total_sessions) * 100)
      : 0;
...
router.put('/:id', (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = Number(req.params.id);
    const existing = db.select().from(playersTable)
      .where(and(eq(playersTable.id, id), isNull(playersTable.deleted_at)))
      .get();
    ...
    const [updated] = db.update(playersTable)
      .set({ name: name.trim() })
