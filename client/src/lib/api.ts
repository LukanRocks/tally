...
export const api = {
  games: {
    ...
    create: (data: Omit<Game, 'id' | 'created_at' | 'session_count' | 'attachments'>) =>
      req<Game>('/games', json('POST', data)),
    update: (id: number, data: Partial<Omit<Game, 'id' | 'created_at'>>) =>
      req<Game>(`/games/${id}`, json('PUT', data)),
    ...
    deleteAttachment: (id: number, aid: number) =>
      req<void>(`/games/${id}/attachments/${aid}`, { method: 'DELETE' }),
  },
  ...
  players: {
    ...
    update: (id: number, name: string) =>
      req<Player>(`/players/${id}`, json('PUT', { name })),
  ...
  sessions: {
    ...
    }) => req<Session>('/sessions', json('POST', data)),
    delete: (id: number) => req<void>(`/sessions/${id}`, { method: 'DELETE' }),
  },

  stats: {
    leaderboard: () => req<LeaderboardEntry[]>('/stats/leaderboard'),
    leaderboardByGame: (gameId: number) =>
      req<LeaderboardEntry[]>(`/stats/leaderboard/game/${gameId}`),
    mostPlayed: () => req<MostPlayedGame[]>('/stats/most-played'),
    headToHead: (player1: number, player2: number) =>
      req<HeadToHead>(`/stats/head-to-head?player1=${player1}&player2=${player2}`),
  },
};
