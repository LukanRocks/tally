...
describe('games', () => {
  ...
  describe('GET /api/v1/games', () => {
    it('includes owner name, type and session count', async () => {
      ...
      const res = await request(await testApp()).get('/api/v1/games')
    ...
    it('honours sort and order parameters', async () => {
      ...
      const byName = await request(await testApp()).get('/api/v1/games?sort=name&order=desc')
      expect(byName.body.map((g: { name: string }) => g.name)).toEqual(['Zendo', 'Azul'])

      const byPrice = await request(await testApp()).get('/api/v1/games?sort=price&order=asc')
    ...
    it('filters by search, player counts and owner', async () => {
      ...
      const search = await request(await testApp()).get('/api/v1/games?search=cat')
      expect(search.body.map((g: { name: string }) => g.name)).toEqual(['Catan'])

      const min = await request(await testApp()).get('/api/v1/games?minPlayers=3')
      expect(min.body.map((g: { name: string }) => g.name)).toEqual(['Catan'])

      const max = await request(await testApp()).get('/api/v1/games?maxPlayers=2')
      expect(max.body.map((g: { name: string }) => g.name)).toEqual(['Chess'])

      const byOwner = await request(await testApp()).get(`/api/v1/games?ownerId=${owner.id}`)
    ...
    it('matches search case-insensitively', async () => {
      ...
      const lower = await request(await testApp()).get('/api/v1/games?search=catan')
      const upper = await request(await testApp()).get('/api/v1/games?search=CATAN')
  ...
  describe('GET /api/v1/games/:id', () => {
    it('returns the game with attachments and session count', async () => {
      ...
      const res = await request(await testApp()).get(`/api/v1/games/${game.id}`)
    ...
    it('404s for unknown and soft-deleted games', async () => {
      ...
      expect((await request(await testApp()).get('/api/v1/games/999')).status).toBe(404)
      expect((await request(await testApp()).get(`/api/v1/games/${game.id}`)).status).toBe(404)
    })
  })
  ...
  describe('DELETE /api/v1/games/:id', () => {
    it('cascades the soft delete to sessions and results', async () => {
      ...
      expect((await request(await testApp()).delete(`/api/v1/games/${game.id}`)).status).toBe(204)
    ...
    it('404s for unknown games', async () => {
      expect((await request(await testApp()).delete('/api/v1/games/999')).status).toBe(404)
    })
  })
  ...
  describe('uploads', () => {
    it('attaches a cover image', async () => {
      ...
      const res = await request(await testApp())
        .post(`/api/v1/games/${game.id}/cover`)
    ...
    it('rejects a cover upload with no file', async () => {
      ...
      const res = await request(await testApp()).post(`/api/v1/games/${game.id}/cover`)
    ...
    it('adds and removes a PDF attachment', async () => {
      ...
      const withAttachment = await request(await testApp()).get(`/api/v1/games/${game.id}`)
      expect(withAttachment.body.attachments).toHaveLength(1)

      const removed = await request(await testApp()).delete(`/api/v1/games/${game.id}/attachments/${created.body.id}`)
      expect(removed.status).toBe(204)

      const after = await request(await testApp()).get(`/api/v1/games/${game.id}`)
    ...
    it('404s when deleting an attachment that belongs to another game', async () => {
      ...
      const res = await request(await testApp()).delete(`/api/v1/games/${other.id}/attachments/${created.body.id}`)
